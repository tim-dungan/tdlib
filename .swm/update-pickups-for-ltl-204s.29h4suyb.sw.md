...
This document will walk you through the update of Pickups for EDI LTL 204s in the RPGLE program located at <SwmPath>[QRPGLESRC/p55#updpu.pgm.rpgle](/QRPGLESRC/p55#updpu.pgm.rpgle)</SwmPath>.

The update focuses on:

1. Handling multiple stops and ensuring only one pickup and one delivery for LTL.
2. Processing and updating pickup numbers and related details.
3. Managing consignee information and ensuring correct assignment based on various conditions.
4. Handling special cases for different EDI partners like <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="222:5:5" line-data="      *         for MACY204 LTL">`MACY204`</SwmToken>, <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="235:10:10" line-data="     c                   IF        thtpid = &#39;STM204LTL&#39;">`STM204LTL`</SwmToken>, and <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="143:8:8" line-data="     c                             THTPID &lt;&gt; &#39;WIN204LTL&#39;                         (T1)">`WIN204LTL`</SwmToken>.

# Initial setup

<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="4">

---

The program begins by defining the necessary files and data structures. This setup is crucial for handling the various records and operations throughout the program.
...
<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="34">

---

Several data structures and variables are defined to store and manipulate the data required for processing the pickups. These include arrays for consignee information, temporary storage for pickup details, and other relevant fields.
...
# Entry point and key lists

<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="94">

---

The entry point of the program and key lists for various operations are defined. These key lists are used for chaining and reading records from the files.
...
<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="124">

---

The program ensures that there are no more than two stops for LTL (one pickup and one delivery). If more than two stops are found, the program updates the status and exits.
...
# Removing and recreating records

<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="148">

---

The program removes relevant records from the PUDT and PUPO files and then recreates them based on the new pickup details.
...
# Terminal and zip code handling

<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="208">

---

The program retrieves terminal and zip code information based on the destination and other conditions. This information is used to update the pickup details accordingly.
...
<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="304">

---

The program retrieves and processes consignee information based on various conditions. This includes handling special cases for different EDI partners and ensuring the correct consignee is assigned.
...
<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="394">

---

The program processes the detail file to build the PUDT and PUPO records. It iterates through the records, calculates the necessary quantities and weights, and updates the files accordingly.
...
# Special processing for stops

<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="547">

---

Special processing is done for stops to handle multiple <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="655:7:7" line-data="      * build the POs for the same consignee into one array --- SameConsPO_ARR">`POs`</SwmToken> and consignees. The program builds arrays for the consignee information and processes them to create the necessary records.
...
# Terminal assignment for <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="222:5:5" line-data="      *         for MACY204 LTL">`MACY204`</SwmToken> and <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="235:10:10" line-data="     c                   IF        thtpid = &#39;STM204LTL&#39;">`STM204LTL`</SwmToken>

<SwmSnippet path="QRPGLESRC/p55#updpu.pgm.rpgle" line="806">

---

The program assigns terminals based on the destination and other conditions for <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="222:5:5" line-data="      *         for MACY204 LTL">`MACY204`</SwmToken> and <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="235:10:10" line-data="     c                   IF        thtpid = &#39;STM204LTL&#39;">`STM204LTL`</SwmToken>. This ensures that the correct terminal is assigned for each pickup.
...
<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="904">

---

The program handles appointment information for <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="222:5:5" line-data="      *         for MACY204 LTL">`MACY204`</SwmToken>, ensuring that the appointment details are correctly updated in the remarks field.
...
<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="934">

---

The program processes PO numbers from the <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="346:24:24" line-data="      * (MCI qualifier in H05 file is a TJMAX LTL if L11 MCI qualifier is a NRCS-L">`L11`</SwmToken> segment and updates the PUPO file accordingly. It ensures that duplicate <SwmToken path="/QRPGLESRC/p55#updpu.pgm.rpgle" pos="655:7:7" line-data="      * build the POs for the same consignee into one array --- SameConsPO_ARR">`POs`</SwmToken> are not added and handles special cases for different EDI partners.
...
# Division and consignee table

<SwmSnippet path="/QRPGLESRC/p55#updpu.pgm.rpgle" line="1104">

---

A table is defined for valid divisions and consignee numbers. This table is used to lookup and assign the correct consignee based on the division and PO number.
...
This concludes the walkthrough of the update for pickups for EDI LTL 204s. The program ensures that pickups are processed correctly, consignees are assigned accurately, and special cases for different EDI partners are handled appropriately.
